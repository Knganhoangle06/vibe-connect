                ...
                @foreach($posts as $post)
                    <div class="card1" style="margin-bottom: 15px;">
                        ...
                        <p style="margin: 10px 0;">
                            {!! preg_replace('/(' . preg_quote($query, '/') . ')/i', '<strong style="background-color: yellow;">$1</strong>', htmlspecialchars($post->content)) !!}
                        </p>

                        @if($post->media_url)
                            @php $mediaSrc = strpos($post->media_url, 'http') === 0 ? $post->media_url : asset('storage/' . $post->media_url); @endphp
